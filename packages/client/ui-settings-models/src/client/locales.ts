...
export const en = {
  ...
  credentialChoice: 'Credential storage',
  credentialCollision: '{reference} is already used by {providers}. Choose where this new API key should be saved.',
  credentialSeparate: 'Use a separate credential ({reference})',
  credentialSeparateHint: 'Keeps the other provider’s API key unchanged.',
  credentialShare: 'Share the existing credential ({reference})',
  credentialShareHint: 'Replaces the API key for every provider using this credential.',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  credentialChoice: '凭据存储方式',
  credentialCollision: '{reference} 已由 {providers} 使用。请选择这次输入的 API 密钥要存到哪里。',
  credentialSeparate: '使用独立凭据（{reference}）',
  credentialSeparateHint: '保留其他提供方当前使用的 API 密钥。',
  credentialShare: '共享现有凭据（{reference}）',
  credentialShareHint: '会替换所有使用该凭据的提供方的 API 密钥。',
  customized: '自定义设置',
  baseUrl: 'API 地址',
