...
解析器保留可见的用户／助手文本和生成摘要。带相同助手 UUID 的重复记录只在其中一份文本是另一份前缀时收敛。Sidechain、系统／进度／文件历史记录、thinking 块、附件、工具输入和工具结果正文都会被排除；工具 id 只贡献不可执行的名称与终态。冲突重复、版本变化、时间戳乱序和未知必要结构会故障关闭。
