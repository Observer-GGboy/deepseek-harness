...
v1 转换器显式接受 `2.1.128` 至 `2.1.222` 的版本，包括长度受限的预发布后缀。缺失、格式错误、过旧或更新的版本都会故障关闭。不支持版本的诊断只会回显简短、规范的纯数字版本；格式错误、预发布、路径形状、凭据形状和超长值绝不会进入错误消息。transcript 可以包含该范围内的多个版本；安全来源信息会以 `min..max` 记录观察到的最小值和最大值，而不是拒绝受支持的更新器过渡。
