...
Converter v1 uses explicit external-version policies instead of accepting arbitrary future rows. Codex accepts `0.144.0` through `0.148.999` (including in-range prereleases), permits repeated consistent `session_meta`, and records all observed supported versions. Claude Code accepts `2.1.128` through `2.1.222`, records supported updater transitions as a minimum/maximum range, and explicitly treats the body-free `frame-link` record observed in 2.1.128 as ignored metadata. Missing, malformed, older, newer, identity-conflicting, or unknown required structures fail closed.
...
The Web Client receives only source metadata, capture counts, cwd hint, safe per-model allowances, and opaque reservation id. It never receives transcript excerpts or file paths. Leaving the section discards an uncommitted reservation; capture and commit accept cancellation. Multiple callers waiting on one identical commit cancel independently, and only the last cancellation aborts the shared work. Reservations are bounded and process-local, while committed idempotency is durable across tabs, processes, restarts, JSONL, and SQLite. The v1 progress contract reports only discover/capture/commit phases; it does not expose byte counters without a real Remote progress seam.

## Verification

Provider fixtures pin metadata-only discovery, symlink exclusion, stable append behavior, deterministic read/rehash/final-stat mutation faults, partial-tail handling, the external version ranges, Codex repeated metadata, Claude Code 2.1.128 `frame-link`, structural exclusion, credential redaction, and path/body-free failures. Host tests pass provider-neutral snapshots through native seed validation, model-specific composition sizing, persistence/attachment fault injection, cancellation and retry, and 2-, 10-, and 100-way contention against real JSONL and SQLite backends; six separate Node processes also contend against each backend. Client tests pin explicit three-way confirmation, stage status, cancellation, metadata-only rendering, and the post-publication accounting warning. The four focused provider/Host/Client suites are held at 100% statements, branches, functions, and lines. A keyless real Loader + Web + Chromium scenario imports a Codex snapshot containing a historical shell-call tripwire, continues it through a replay model, verifies no `tool/call` was synthesized or executed, and commits the resulting accessibility golden.
